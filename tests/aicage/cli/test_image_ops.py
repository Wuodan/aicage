...
class DockerInvocationTests(TestCase):
    def test_pull_image_success_and_warning(self) -> None:
        ...
        with mock.patch("aicage.registry.base_image.subprocess.run", return_value=pull_ok) as run_mock:
        ...
        with mock.patch("aicage.registry.base_image.subprocess.run", side_effect=[pull_fail, inspect_ok]):
    ...
    def test_pull_image_raises_on_missing_local(self) -> None:
        ...
        with mock.patch("aicage.registry.base_image.subprocess.run", side_effect=[pull_fail, inspect_fail]):
    ...
    def test_discover_local_bases_and_errors(self) -> None:
        ...
        with mock.patch("aicage.registry.base_image.subprocess.run", return_value=FakeCompleted(stdout=list_output, returncode=0)):
            aliases = base_image._discover_local_bases("repo", "codex")
        self.assertEqual(["debian", "ubuntu"], aliases)

        with mock.patch(
            "aicage.registry.base_image.subprocess.run",
